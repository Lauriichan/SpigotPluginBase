...
final class ComponentBuilderUtils {
    ...
    private static class ColorResult {
        ...
        public ColorResult(Color color, int length) {
            this.color = color;
            this.length = length;
        }
        ...
        if (message.hoverAction() != null) {
            ...
            case HOVER_SHOW:
                if (hoverAction.getValue() instanceof ItemStack) {
                    hover = new HoverEvent(HoverEvent.Action.SHOW_ITEM, ComponentHelper.get().hoverItem((ItemStack) hoverAction.getValue()));
                } else if (hoverAction.getValue() instanceof Entity) {
                    hover = new HoverEvent(HoverEvent.Action.SHOW_ITEM, ComponentHelper.get().hoverEntity((Entity) hoverAction.getValue()));
                }
            default:
                break;
            }
