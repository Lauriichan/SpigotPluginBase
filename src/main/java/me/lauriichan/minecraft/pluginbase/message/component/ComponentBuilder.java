...
public abstract class ComponentBuilder<P extends ComponentBuilder<?, ?>, S extends ComponentBuilder<P, S>> implements ISendable {
    ...
    public SubComponentBuilder<S> appendContent(final Actor<?> actor, final IMessage message, final Key... placeholders) {
        return appendContent(actor.getMessageManager().format(message, placeholders));
    }

    public SubComponentBuilder<S> appendContent(final Actor<?> actor, final MessageProvider provider, final Key... placeholders) {
        return appendContent(actor.getMessageManager().translate(provider, actor.getLanguage(), placeholders));
    }

    public SubComponentBuilder<S> appendContent(final Actor<?> actor, final String messageId, final Key... placeholders) {
        return appendContent(actor.getMessageManager().translate(messageId, actor.getLanguage(), placeholders));
    }
    ...
    public SubComponentBuilder<S> appendContent(ComponentBuilder<?, ?> builder) {
        ...
        if (builder instanceof SubComponentBuilder) {
            append.loadFrom((SubComponentBuilder<?>) builder);
        }
        if (builder.isEmpty()) {
            return append;
        }
